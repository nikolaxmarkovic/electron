...
```javascript @ts-type={fetchGrantedDevices:()=>(Array<Electron.USBDevice|Electron.HIDDevice|Electron.SerialPort>)}
...
```javascript @ts-type={fetchGrantedDevices:()=>(Array<Electron.USBDevice|Electron.HIDDevice|Electron.SerialPort>)} @ts-type={updateGrantedDevices:(devices:any)=>void}
const { app, BrowserWindow } = require('electron')
